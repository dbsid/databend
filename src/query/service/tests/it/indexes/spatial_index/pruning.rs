...
fn point_wkb(x: f64, y: f64) -> Result<Vec<u8>> {
    geo_to_ewkb(Geometry::from(Point::new(x, y)), Some(4326))
}
...
async fn test_spatial_index_pruning_geometry() -> anyhow::Result<()> {
    ...
    let total_blocks = block_points.len();
    ...
    for (wkt, expected_blocks) in &cases {
        let filter = format!("st_intersects(g, to_geometry('{wkt}', 4326))");
    ...
    for (wkt, expected_blocks) in &cases {
        let and_filter = format!("st_intersects(g, to_geometry('{wkt}', 4326)) AND id > 0");
        let push_down = Some(PushDownInfo {
        ...
        let or_filter = format!("st_intersects(g, to_geometry('{wkt}', 4326)) OR id > 0");
        let push_down = Some(PushDownInfo {
